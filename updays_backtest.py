...
==============================
规则：
  - WTS 信号 = 1（周趋势看涨）AND 上周上涨天数（收盘>=开盘）>= 3 → 周一开盘入场
  - WTS 信号 = 0 OR 上周上涨天数 < 3 → 周一不进入，保持空仓
  - 每周五收盘必须清仓（无论信号如何）
  - 周内（周二～周四）持仓不变

对比策略：
  1. pos_updays  — 本策略（WTS + up_days>=3 + 周五清仓）
  2. pos_wts     — 纯WTS（无 up_days 过滤，无强制周五清仓）
  3. pos_asym    — 非对称 WTS+DTS（来自 asym_backtest.py）
  4. BNH         — 买入持有
"""
...
COST_BPS=15
UP=1; DOWN=-1; FLAT=0
UP_DAYS_THR = 3     # 上周上涨天数门槛

# 各ETF最优日线参数（用于非对称对比策略）
OPT_PARAMS = {
    "上证50ETF":  (0.75, 0.60),
    "沪深300ETF": (2.50, 2.00),
    "中证500ETF": (2.50, 2.00),
...
def build_signals(daily, d_same, d_rev):
    ...
    def agg_w(g):
        ...
        up_days = (g['close'] >= g['open']).sum()       # 本周上涨天数
        return pd.Series({
            'we':    g['date'].iloc[-1],
            'open':  g['open'].iloc[0],
            'high':  g['high'].max(),
    ...
    # week_to_perm[str(week_key)] = WTS signal of previous week
    week_to_perm   = {str(wk_list[i+1]): wts_list[i]  for i in range(len(wk_list)-1)}
    # week_to_updays[str(week_key)] = up_days count of previous week
    week_to_updays = {str(wk_list[i+1]): upd_list[i]  for i in range(len(wk_list)-1)}

    d['wp']      = d['wk'].apply(lambda w: week_to_perm.get(str(w), 0))
    d['prev_up'] = d['wk'].apply(lambda w: week_to_updays.get(str(w), 0))
    ...
    # ── 策略4：WTS + up_days>=3 + 周五强制清仓 ────────────────────────────────
    # 入场条件：周一（weekday=0），wp=1，prev_up>=UP_DAYS_THR
    # 持仓：周二～周四维持
    # 出场：周五（weekday=4）强制清仓
    upd_pos = []; pos_u = 0
    for i, row in d.iterrows():
        ...
        if wd == 0:   # 周一：根据入场条件决定
            if wp == 1 and pup >= UP_DAYS_THR:
                pos_u = 1
            else:
                pos_u = 0
        elif wd == 4:  # 周五：强制清仓（持仓到当日收盘）
            # 维持持仓直到当天结束再清（清仓发生在下一个bar的开盘）
            # 用一个特殊标记：pos_u 先维持，收盘后清仓
            # 实际上我们在回测中处理：周五 end-of-day 强制平仓
            upd_pos.append(pos_u)
            pos_u = 0   # 周五收盘后清仓（对下一个bar生效）
            continue
        # 其他时候（周二～周四）维持当前仓位（wp=0 时强制空仓）
        if wp == 0:
            pos_u = 0
        upd_pos.append(pos_u)

    d['pos_updays'] = upd_pos

    return d, wk


def backtest_seg(daily_full, pos_col, start, end, init=1_000_000.0, friday_force_close=False):
    """
    friday_force_close: 若 True，每个周五收盘时强制平仓（日内持仓，收盘卖出）
    """
    d = daily_full[
    ...
    for i, row in d.iterrows():
        ...
        if target != pos:
            ...
            if target == 1:
                ...
                cash   = 0.0
                ep     = o * (1 + COST_BPS / 10000)
                ed     = row['date']

        pos = target
        equity.append(cash + shares * c)

        # 周五收盘后强制平仓（持仓到收盘，收盘价卖出）
        if friday_force_close and wd == 4 and pos == 1 and shares > 0:
...
# ── 主流程 ────────────────────────────────────────────────────────────────────
STRATS = [
    ('pos_updays', '上涨≥3天WTS', True),   # (col, label, friday_force_close)
    ('pos_wts',    '纯WTS',       False),
    ('pos_sym',    '对称DTS',     False),
    ('pos_asym',   '非对称DTS',   False),
]

all_data = {}

print(f"{'标的':10s} {'阶段':8s}  "
      f"{'上涨≥3+WTS':>12s}  {'纯WTS':>10s}  "
      f"{'对称DTS':>10s}  {'非对称DTS':>10s}  {'BNH':>8s}")
print("─" * 85)
...
for name in ETF_FILES:
    ...
    for plabel, ps, pe in PERIODS:
        results = {}
        for col, lbl, fri in STRATS:
            r = backtest_seg(dsig, col, ps, pe, friday_force_close=fri)
            results[col] = r
        ...
        if results['pos_wts']:
            ...
            mu  = results['pos_updays']['metrics']
            print(f"{name:10s} {plabel:8s}  "
                  f"UPD {mu['total_return']:+6.1f}%(α{mu['alpha']:+.1f})  "
...
out = OUTPUT / 'updays_results.json'
